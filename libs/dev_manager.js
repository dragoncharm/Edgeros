...
class Dev extends EventEmitter {
    ...
    validLightSupport(devid) {
        const dev = this.getDev(devid)
        if (dev) {
            return dev.type === 'light.belt' && dev.brand === '核芯科技HXKJ' && dev.model === 'RGB-01'
        }
    }
    validCurtainSupport(devid) {
        const dev = this.getDev(devid)
        if (dev) {
            return dev.type === 'curtains' && dev.brand === '核芯科技HXKJ' && dev.model === 'CL01'
        }
    }
    validSensorSupport(devid) {
        const dev = this.getDev(devid)
        if (dev) {
            return dev.type === 'sensor.pm2.5' && dev.brand === '核芯科技HXKJ' && dev.model === 'PM2.5-01'
        }
    }
    validPlugSupport(devid) {
        const dev = this.getDev(devid)
        if (dev) {
            return dev.type === 'plug' && dev.brand === '核芯科技HXKJ' && dev.model === 'CZ02A-J'
        }
    }
    validSensorCH4Support(devid) {
        const dev = this.getDev(devid)
        if (dev) {
            return dev.type === 'sensor.ch4' && dev.brand === '核芯科技HXKJ' && dev.model === 'TRQ-02'
        }
    }
}
